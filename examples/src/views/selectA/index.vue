  ...
  <div>
    ...
    <kd-select-a v-model="val" multiple collapse-tags title="基础用法" :options="options">
      <template slot-scope="{ item }">
        <!-- @click.prevent.native 解决点击checkbox逻辑错误 -->
        <el-checkbox :disabled="item.disabled" @click.prevent.native>{{ item.label }}</el-checkbox>
      </template>
    </kd-select-a>
    <h6>多选自定义模版用法</h6>
    <kd-select-a v-model="val" multiple collapse-tags title="基础用法" :options="options">
      <template slot-scope="{ item }">
        <div>
          <span style="float: left">
            <el-checkbox :disabled="item.disabled" @click.prevent.native>{{ item.label }}</el-checkbox></span
          >
