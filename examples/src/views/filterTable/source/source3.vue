    ...
    <template #operation>
      <div class="right_btn">
        <kd-popover-button
          ...
          :reference-text="mBatchTitle('删除', checkoutList)"
          :reference-btn-attrs="{ style: 'background: #ebedf0;' }"
        ...
        <kd-popover-button
          ...
          :reference-text="mBatchTitle('暂停', checkoutList)"
          :popover-attrs="{ title: '批量暂停' }"
          :reference-btn-attrs="{ style: 'background: #ebedf0;' }"
        ...
        <kd-popover-button
          ...
          :reference-text="mBatchTitle('下线', checkoutList)"
          :popover-attrs="{ title: '批量下线' }"
          :reference-btn-attrs="{ type: 'primary' }"
...
export default {
    ...
    return {
      ...
      columnsData: [
        ...
        { title: '任务版本', key: 'version', render: (h, params) => h('span', {}, `v${params.row.version}`) },
        {
          ...
          width: '160px',
          render: (h, params) =>
            h('span', {}, params.row.principals.map((own) => `${own.username}(${own.realName})`).join(', ')),
        },
  ...
  computed: {
    ...
    offsDisable() {
      const flag = this.checkoutList.some(
        (one) => one.status !== 'PAUSED' && one.status !== 'FAILED' && one.status !== 'UNSTART'
      );
      return !this.checkoutList.length || flag;
    },
  },
