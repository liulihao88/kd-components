...
export default {
  ...
  watch: {
    ...
    data(newVal) {
      ...
      const keys = this.getInitCheckedKeys(newVal);
  ...
  methods: {
    ...
    getInitCheckedKeys(datas) {
      const keys = datas.reduce((conn, data) => {
        if (data.checked != null && data.checked === true) {
          conn.push(data[this.nodeKey]);
        }
        if (data.children && data.children.length > 0) {
          const subKeys = this.getInitCheckedKeys(data.children);
...
.el-tree .is-current .ivu-tree-title {
  color: #409eff;
  font-weight: 700;
}
