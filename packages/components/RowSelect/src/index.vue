    ...
    >
      <template v-if="multiple && showAll">
        <slot name="all" :index="'$all'" :active="selectAll">
          <div class="item" :class="{ 'is-active': selectAll }" data-index="$all" :style="itemStyles">全部</div>
        </slot>
      </template>
      <div v-for="(item, index) in options" :key="item[keyName]">
        <slot :item="item" :index="item[keyName]" :active="selectedKey.includes(item[keyName])">
          <div
            class="item"
            :class="{
...
export default {
    ...
    selectedKey: {
      handler(value) {
        if (this.multiple) {
          ...
          this.$emit('selectChange', this.options.filter((x) => this.selectedKey.includes(x[this.keyName])) || []);
        } else {
          this.$emit('input', value[0] || '');
          this.$emit('selectChange', this.options.filter((x) => this.selectedKey.includes(x[this.keyName]))[0] || '');
        }
      },
      deep: true,
      immediate: true,
  ...
  methods: {
    onResize() {
      this.optionScrollHeight = Math.max(this.$refs.optionsBox.scrollHeight - 8, this.lineHeight) + 'px';
    },
    onItemClick(e) {
      ...
      if (this.required) {
        if (this.multiple) {
          if (!Array.isArray(this.value)) {
            console.error('kd-row-select：多选配置下需要 v-model 绑定数组类型');
            return;
          }
          ...
          if (typeof this.value !== 'string') {
            console.error('kd-row-select：单选配置下需要 v-model 绑定字符串类型');
            return;
          }
          if (this.value === '') {
            this.add(this.options[0][this.keyName]);
